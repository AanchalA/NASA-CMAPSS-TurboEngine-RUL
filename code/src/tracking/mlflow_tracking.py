...
def configure_mlflow():
    ...
    mlflow.set_experiment(
        getenv("CMAPSS_MLFLOW_EXPERIMENT", "cmapss-preprocessing")
    )
...
def log_preprocessing_state(artifacts):
    mlflow.log_dict(
        ...
        PREPROCESSING_STATE_ARTIFACT,
    )


def load_preprocessing_model(run_id):
    return mlflow_spark.load_model(
        f"runs:/{run_id}/{PREPROCESSING_PIPELINE_ARTIFACT}"
    )


def load_preprocessing_state(run_id):
    data = mlflow_artifacts.load_dict(
        f"runs:/{run_id}/{PREPROCESSING_STATE_ARTIFACT}"
    )
